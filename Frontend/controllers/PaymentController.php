<?php
require_once 'frontend/models/Oder.php';
require_once 'frontend/models/OderDetail.php';
...
class PaymentController extends Controller
{
    public  function index()
    {
        if(isset($_POST["submit"]))
        {
            ...
            if(empty($fullname))
            {
                $this->error['fullname']=" * Không được để trống họ tên";
            }
            if(empty($address))
            {
                $this->error['address']=" * Không được để trống họ địa chỉ";
            }
            if(empty($mobile))
            {
                $this->error['mobile']=" * Không được để trống SDT";
            }
            if(empty($this->error))
            {
...
//        echo "</pre>";
//        die();
        $this->content=$this->render("frontend/views/payments/index.php");
        require_once 'frontend/views/layouts/main.php';
    }
    public function sendMail($email)
    {
        // Instantiation and passing `true` enables exceptions
        $this->content=$this->render('frontend/views/payments/OderDetailMail.php');
...
//
    ...
    {
        $this->content=$this->render("frontend/views/payments/thank.php");
        require_once 'frontend/views/layouts/main.php';
    }
